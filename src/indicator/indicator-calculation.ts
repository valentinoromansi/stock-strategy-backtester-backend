...
export function smaValue(smaPeriod: number, slice0: VerticalSlice): number {
  if (!slice0.hasConnectedSlices(Direction.LEFT, smaPeriod - 1) || smaPeriod <= 0) return null
  let sma: number = 0
...
export function emaValue(emaPeriod: number, slice: VerticalSlice): number {
  if (!slice.hasConnectedSlices(Direction.LEFT, emaPeriod * 2 - 4) || emaPeriod <= 0) return null
  const sma = smaValue(emaPeriod, slice.getConnectedSlice(Direction.LEFT, emaPeriod - 1))
  const multiplier = 2 / (emaPeriod + 1)
  const k = 2 / (emaPeriod + 1)
  let ema: number = 0
  for (var i = 1; i < emaPeriod; i++) {
    const prevEmaValue = i == 1 ? sma : ema
    const curSlice = slice.getConnectedSlice(Direction.LEFT, emaPeriod - 1 - i).close
...
export function rsiValue(rsiPeriod: number, slice: VerticalSlice): number {
  if (!slice.hasConnectedSlices(Direction.LEFT, rsiPeriod) || rsiPeriod <= 0) return null
  let gainSum = 0
  let lossSum = 0
  slice.executeEachIteration(Direction.LEFT, rsiPeriod - 1, (slice) => {
    const prevSlice = slice.getConnectedSlice(Direction.LEFT, 1)
