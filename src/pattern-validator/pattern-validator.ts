...
import { StrategyRule } from "../strategy/strategy-rule"
...
export function isPatternValid(slice: VerticalSlice, strategyRules: StrategyRule[]): boolean {
  ...
  for (let i = 0; i < strategyRules.length; ++i) {
    ...
    const val1 = slice.getConnectedPrice(Direction.RIGHT, rule.valueData1.id).getValueRelativeToAttributes(rule.valueData1)
    const val2 = slice.getConnectedPrice(Direction.RIGHT, rule.valueData2.id).getValueRelativeToAttributes(rule.valueData2)
